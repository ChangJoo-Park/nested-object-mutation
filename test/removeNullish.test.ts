...
describe("removeNullish 2 depth =>", () => {
  ...
  test("Returns empty object with one undefined value", () => {
    expect(nestedObjectMutation.removeNullish({ a: { b: undefined } })).toEqual({ a: {} })
  });

  test("Returns empty object with undefined and null both", () => {
    expect(nestedObjectMutation.removeNullish({ a: { b: undefined, c: null } })).toEqual({ a: {} })
  });
  ...
  test("Returns empty object with complex keys", () => {
    ...
    expect(nestedObjectMutation.removeNullish(source)).toEqual({ b: {}, e: 'Hello World' })
  });
});
...
describe("removeNullish 3 depth =>", () => {
  test("Returns empty object with deep nested null", () => {
    ...
    expect(nestedObjectMutation.removeNullish(source)).toEqual({ a: { b: {} } } )
  });
